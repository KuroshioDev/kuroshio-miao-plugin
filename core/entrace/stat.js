class statApp {
  constructor(app, ctx, config) {
    ctx.guild().command('genshin.miao.stat', { authority: 1 })
    ...
    ctx.guild().command('genshin.miao.abysspct', { authority: 1 })
    .alias('#深渊12层使用率')
    ...
    ctx.guild().command('genshin.miao.abyssteam', { authority: 1 }).userFields(['id'])
    .shortcut(/#深渊(组队|配队)/)
    .action(async ({session}) => {
      let application = new app.stat(ctx, session, config)
    ...
    ctx.guild().command('genshin.miao.uploaddata', { authority: 1 }).userFields(['id'])
